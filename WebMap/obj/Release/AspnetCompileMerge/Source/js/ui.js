...
define([
    ...
    Point
      ...
      _init : function() {
         ...
         // info window
         this.map.infoWindow.resize(435, 500);
      ...
      _buildPageList : function() {
         ...
         if (this.config.showWeather) {
          ...
          //sites
         if (this.config.showSites) {
             this.siteLayer = new Sites(this.config);
             this.siteLayer.map = this.map;
             this.siteLayer.ui = this;
             this.siteLayer.startup();
         }
      ...
      createJumpButton: function () {
          ...
          on(myBtn, 'click', lang.hitch(this, this._test, 1));
      },

      _test: function (options) {
          var bob = 1;
      },
      // create page
      ...
      _renderResults : function(pageObj) {
            ...
            if (pageObj.proximityFeatures) {
               ...
               var rgb = color.toRgb();
               var symML = new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color([0, 0, 0, 1]), 2);
               var symM = new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_CIRCLE, 22, symML, color);
               var symD = new SimpleMarkerSymbol(SimpleMarkerSymbol.STYLE_DIAMOND, 25, symML, color);
               ...
               for (var i=0; i<pageObj.proximityFeatures.length; i++) {
                   ...
                   if (attr.hasOwnProperty("TYPE")) {
                       ...
                       if (iconFile.icons.hasOwnProperty(type)) {
                           var symPic = new PictureMarkerSymbol(iconFile.icons[type].map, 32, 37);
                           symPic.setOffset(0, 18);
                           this.map.graphics.add(new Graphic(pt, symPic, attr, infoTemp));
                       } else {
                       ...
                       if (attr.hasOwnProperty("num_bedrooms") && attr.hasOwnProperty("business_type")) {
                           symText.text = attr.num_bedrooms;
                           switch (attr.business_type) {
                               case 'commercial':
                                   this.map.graphics.add(new Graphic(pt, symD, attr, infoTemp));
                                   break;
                               case 'private':
                                   this.map.graphics.add(new Graphic(pt, symM, attr, infoTemp));
                                   break;
